...
from qi.tool.account_loader.constant import qi_acc_reader,attr_reader
...
def get_parent():
    sql = '''
            select strategy_id as account_id, parents 
            from allocation 
            where parents != '0'
    '''
    parent = pd.read_sql(sql, attr_reader)
    return parent
...
def get_pos(date_):
    ...
    sql = f'''
            select account_id, date, symbol, volume 
            from position 
            where date = '{date_}' and volume!=0
            and account_id not like '%%_OTHER' and account_id not like '%%_T0' 
            and account_id not like '%%_ARGO' and account_id not like '%%_PIPE' 
            and account_id not like '%%_OHTER'
           '''
    pos = pd.read_sql_query(sql, qi_acc_reader)
    parent = get_parent()
    pos = pos.merge(parent, how='left', on='account_id')
    pos.account_id[pos.parents.notna()] = pos.parents[pos.parents.notna()]
    pos = pos[pos.account_id.str.contains('_')]
    if not pos.empty:
...
def get_trade(date_):
    ...
    sql = f'''
            ...
            and account_id not like '%%_OHTER'
           '''
    trade = pd.read_sql_query(sql, qi_acc_reader)
    parent = get_parent()
    trade = trade.merge(parent, how='left', on='account_id')
    trade.account_id[trade.parents.notna()] = trade.parents[trade.parents.notna()]
    trade = trade[trade.account_id.str.contains('_')]
    if not trade.empty:
    ...
    return trade.drop(['parents'], axis=1)

def get_jbm_500(pos_, trade_):
    '''
    将分仓后的持仓、交易数据中的JBM复制为JBM_500
    '''
    temp_pos = pos_[pos_.strategy_id.str.contains('JBM')]
    if not temp_pos.empty:
        temp_pos['strategy_id'] = temp_pos.apply(lambda x : x['strategy_id'].replace('JBM','JBM500'),axis=1)
        pos = pos_.append(temp_pos)
    else:
        pos = pos_
    temp_trade = trade_[trade_.strategy_id.str.contains('JBM')]
    if not temp_trade.empty:
        temp_trade['strategy_id'] = temp_trade.apply(lambda x : x['strategy_id'].replace('JBM','JBM500'),axis=1)
        trade = trade_.append(temp_trade)
    else:
        trade = trade_
    return pos, trade
...
def get_product_info():    
    # sql = """
    #     SELECT distinct on (product_id, sec_type) product_id,product_name,trader,commission,sec_type,
    #     institution,min_commission,root_product_id
    #     FROM "public"."product" 
    #     where status='running' 
    #     order by product_id,sec_type,update_time desc
    # """    
    sql = """
            SELECT distinct on (product_id, sec_type) product_id,product_name,trader,commission,sec_type,
            institution,min_commission,margin_rate,root_product_id
            FROM "public"."product_fee" 
            where status='running' 
            order by product_id,sec_type
          """
    return attributiondb.read(sql)
...
def get_alloction():
    sql = """
        SELECT distinct on (strategy_id) strategy_id,strategy_name,product_id 
...
# def get_account_detail(date_, new_account=None):
#     engine_nav = create_engine('mssql+pyodbc://navuser:navqwe@10.144.64.71/JasperNAV?driver=SQL+Server')
#     con_nav = engine_nav.raw_connection()
#     sql = f"""
#             SELECT Account as account_id, TotalAsset as totalasset 
#             FROM JasperAccountDetail 
#             where TotalAsset>0 and Trade_dt='{date_}'
#         """
#     ret = pd.read_sql_query(sql, con_nav)
#     if not new_account is None:
#         ret = ret.append(new_account, ignore_index=True)
#     return ret
...
def get_performance(from_, to_):
    ...
    sql = f'''
        SELECT strategy_name,sum(pnl) as pnl,sum(bm_pnl+trade_bm_pnl_buy-trade_bm_pnl_sell) as bm_pnl,sum(stock_mv) as stock_mv
        FROM "performance_1" b 
...
def get_history_alpha(from_, to_):
    sql = f'''
        select trade_dt, strategy_name, alpha, turnover from (
            select a.trade_dt, strategy_name, round(sum(alpha*abs(weight))/sum(abs(weight)),2) as alpha, 
            round(sum(a.stock_turnover/2*weight)/sum(weight),2) as turnover
            from "performance_1" a, "classcification_detail" b 
            where a.trade_dt=b.trade_dt and a.strategy_id=b.strategy_id 
            and a.trade_dt between '{from_}' and '{to_}' 
            and a.cal_type!='3' and special=0
...
def get_daily_alpha(date_):
    ...
    sql = f'''
        select trade_dt,strategy_name,alpha,turnover from (
            select a.trade_dt, strategy_name, round(sum(alpha*abs(weight))/sum(abs(weight)) ,2) as alpha, 
            round(sum(a.stock_turnover/2*stock_mv)/sum(stock_mv) ,2) as turnover
            from "performance_1" a 
            where a.cal_type!='3' and a.trade_dt = '{date_}'
            GROUP BY a.trade_dt,strategy_name
            union
            select trade_dt, strategy_name, round(sum(alpha*abs(weight))/sum(abs(weight)) ,2) as alpha, 
            0 as turnover 
            from "performance_1" a 
            where a.cal_type='3' and trade_dt = '{date_}'
